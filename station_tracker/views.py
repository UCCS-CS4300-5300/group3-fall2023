...
from .forms import UserCreationForm, LoginForm, GasStationOwnerForm, GasStationListingForm, GasStationReviewForm, CustomerInquiryForm
...
def create_gas_station_listing(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Gas station listing created successfully')
...
def update_gas_station_listing(request, listing_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Gas station listing updated successfully')
            return redirect('gas_station_list')  # Redirect to a success page
        else:
...
def create_gas_station_review(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Gas station review created successfully')
...
def update_gas_station_review(request, review_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Gas station review updated successfully')
            return redirect('/')  # Redirect to a success page
        else:
